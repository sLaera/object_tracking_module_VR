...
def get_bop_challange_test_data(bop_dir, dataset, target_obj_id, data_folder='test'):
    ...
    for scene_id, im_id, obj_id, inst_count in target_list:
        ...
        if current_scene_id != scene_id:
            scene_params = inout.load_scene_camera(
                os.path.join(bop_dataset_dir, data_folder, "{:06d}".format(scene_id), "scene_camera.json"))
            scene_gt_fn = os.path.join(bop_dataset_dir, data_folder, "{:06d}".format(scene_id), "scene_gt.json")
            scene_gt_info_fn = os.path.join(bop_dataset_dir, data_folder, "{:06d}".format(scene_id),
                                            "scene_gt_info.json")
            ...
            for counter, gt in enumerate(gts):
                if int(gt['obj_id']) == target_obj_id:
                    ...
                    if visib_fract > visib_thershold:
                        if (dataset == "itodd"):
                            rgb_fn = os.path.join(bop_dataset_dir, data_folder, "{:06d}".format(scene_id), "gray",
                                                  "{:06d}.tif".format(im_id))
                        else:
                            rgb_fn = os.path.join(bop_dataset_dir, data_folder, "{:06d}".format(scene_id), "rgb",
                                                  "{:06d}.png".format(im_id))
                        rgb_files_per_obj[target_obj_id - 1].append(rgb_fn)
                        depth_files_per_obj[target_obj_id - 1].append(
                            os.path.join(bop_dataset_dir, data_folder, "{:06d}".format(scene_id), "depth",
                                         "{:06d}.png".format(im_id)))

                        mask_fn = os.path.join(bop_dataset_dir, data_folder, "{:06d}".format(scene_id), "mask",
                                               "{:06d}_{:06d}.png".format(im_id, counter))
                        mask_visib_fn = os.path.join(bop_dataset_dir, data_folder, "{:06d}".format(scene_id),
                                                     "mask_visib", "{:06d}_{:06d}.png".format(im_id, counter))
...
def get_dataset(bop_dir, dataset, train=True, incl_param=False, eval_model=False, data_folder='None',
                data_per_obj=False, train_obj_visible_theshold=0.1):
    """
                    ...
                    if data_per_obj:
                        ...
                        for counter, gt in enumerate(gts):
                            ...
                            if visib_fract > visib_thershold:
                                ...
                                mask_fn = os.path.join(current_dir + "/mask",
                                                       "{:06d}_{:06d}.png".format(im_id, counter))

                                mask_visib_fn = os.path.join(current_dir + "/mask_visib",
                                                             "{:06d}_{:06d}.png".format(im_id, counter))
