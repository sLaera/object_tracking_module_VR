...
def main(configs):
    ...
    if Detection_reaults != 'none':
        if configs.get('detector', None) != 'MASKRCNN':
            from get_detection_results import get_detection_results, get_detection_scores
        elif configs.get('detector', None) == 'MASKRCNN':
    ...
    for batch_idx, (data, entire_masks, masks, Rs, ts, Bboxes, class_code_images, cam_Ks) in enumerate(
        ...
        class_code_images = class_code_images.detach().cpu().numpy().transpose(0, 2, 3, 1).squeeze(axis=0).astype(
            'uint8')
        ...
        for counter, (r_GT, t_GT, Bbox, cam_K) in enumerate(zip(Rs, ts, Bboxes, cam_Ks)):
            if ignore_bit != 0:
                R_predict, t_predict, success = CNN_outputs_to_object_pose(pred_masks[counter],
                                                                           pred_code_images[counter][:, :,
                                                                           :-ignore_bit],
                                                                           Bbox, BoundingBox_CropSize_GT,
                                                                           divide_number_each_itration,
                                                                           new_dict_class_id_3D_points,
            ...
            if success:
                ...
                depth_image = np.asarray(Image.open(test_depth_files[obj_id][batch_idx])).copy()
                ...
                if configs['use_icp']:
                    ...
                    R_refined, t_refined = icp_refiner.refine_poses(t_predict * 0.1, R_predict, full_mask, depth_image,
                                                                    cam_K.cpu().detach().numpy())
                    R_predict = R_refined
